...
class AdminTest extends DuskTestCase
{
    ...
    public function test_宣言凍結、解除、凍結した宣言は管理者以外は見れない()
    {
        $this->browse(function ($browser) {
            $browser->loginAs($this->admin_user)
                    ->visitRoute('declaration.show',['id' => '90'])
                    ->assertRouteIs('declaration.show',['id' => '90'])
                    ->clickLink('･･･')
                    ->press('凍結')
                    ->waitForDialog($seconds = null)
                    ->assertDialogOpened('本当によろしいですか？')
                    ...
                    ->assertRouteIs('declaration.show',['id' => '90']) //管理者は凍結した宣言の詳細を見れる
            ...
            $browser->loginAs($this->another_user)
                    ...
                    ->visitRoute('declaration.show',['id' => '90']) //一般ユーザーは凍結した宣言詳細を見れない
                    ->assertSee('403');
            ...
            $browser->loginAs($this->admin_user)
                    ...
                    ->assertRouteIs('declaration.show',['id' => '90']) //管理者は凍結解除した宣言詳細を見れる
                    ->assertSee('宣言詳細','宣言者','宣言日','更新日','期間','Do it');

            $browser->loginAs($this->another_user)
                    ->visitRoute('declaration.show',['id' => '90']) //一般ユーザーは凍結解除した宣言詳細を見れる
                    ->assertSee('宣言詳細','宣言者','宣言日','更新日','期間','Do it');
        });
    }
}
